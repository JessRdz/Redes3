...
def user_input(snmpEngine):
    ...
    print("\n")
    vserion = input("Please Enter SNMP Version [OPTION: 1,2,3] :")
    print("\n")
    ip_type = input("Please IP Type [OPTION: 4, 6] :")
    print("\n")
    if vserion in ['1', '2']:
        COMMUNITYSTRING = input("Please Provide SNMP V1/V2 community "
                                  "String: ")
        print("\n")
        config.addV1System(snmpEngine, COMMUNITYSTRING, COMMUNITYSTRING)
        while 1:
            asn = input("Waant to add a another community ("
                                        "Yes/No/n/y)?")
    ...
    if _new_mib_path and _new_mib_path[-1] == "/":
        ...
        CUSTOM_MIB_PATH = _new_mib_path+'/'+','+CUSTOM_MIB_PATH
    LOAD_MIB_MODULE = input("Please provide the custom MIB Name seperated "
                              "by comma: ")
    print("\n")
...
def add_transport(snmpEngine, PORT, ip_type):
    ...
    except error.GenError as excep:
